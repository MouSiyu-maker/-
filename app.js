import {
  ...
  getFilteredContents,
  getContentDetail,
  getExperienceDestinationSummary,
  getExperienceMode,
  getExperienceRoute,
  getGeneratedPlan,
  getGenerationRoute,
  GENERATION_LOADING_DURATION_MS,
  getPaintingRelatedImage,
  getPreferenceGroups,
  getRelatedContents,
  getSearchResults,
  getSeedHeroRecommendation,
  hotspots,
  lightUpSkill,
  paintingData,
  products,
...
let lastViewKey = '';
let generationLoadingTimer = null;

const ICON_PATH = './assets/icons/';
const SEED_HOME_PATH = './assets/figma-seed-home/';
...
function iconImg(file, className = 'ui-icon') {
  return `<img class="${className}" src="${ICON_PATH}${file}" alt="" aria-hidden="true" />`;
}

function seedHomeImg(file) {
  return file.startsWith('./') ? file : `${SEED_HOME_PATH}${file}`;
}

function systemBar(className = '') {
  return `<div class="system-bar ${className}" aria-hidden="true">${iconImg('SystemBar 系统栏.svg', 'system-bar-img')}</div>`;
}
...
const paintingViews = [
  {
    ...
    scenePlace: '葛仙村',
    dimensions: '物艺人景情',
    image: './assets/figma-painting-tab/painting-qingming_.png',
    hotspots,
    related: [
  ...
  },
  {
    ...
    scenePlace: '灵宝夜市',
    dimensions: '桥市舟铺人',
    image: './assets/figma-painting-tab/painting-gusu_.png',
    hotspots: [
...
function render() {
  ...
  tabbar.innerHTML = tabs.map(tab => `
    <button class="tab ${state.tab === tab.id ? 'active' : ''}" data-tab="${tab.id}">
      ${iconImg(TAB_ICON_FILES[tab.id], 'tab-icon')}<b>${tab.label}</b>
    </button>
  `).join('');

  if (state.page === 'detail') renderDetail();
  else if (state.page === 'preference') renderPreference();
  else if (state.page === 'loading') renderGenerationLoading();
  ...
  syncGenerationLoadingTimer();
}

function renderSeed() {
  const list = getFilteredContents(state);
  const [province, city] = getDestinationDisplay(state.selectedContentId);
  const hero = getSeedHeroRecommendation(state.selectedContentId);
  screen.innerHTML = `
    ${systemBar()}
    <header class="seed-appbar">
      ...
      <button class="location-pill" data-open-sheet="location" aria-label="切换地址">
        <img class="location-pin-icon" src="./assets/icon/位置.svg" alt="" aria-hidden="true" />
        <b>${province}</b>
        <b>${city}</b>
        ${iconImg('Frame.svg', 'location-arrow-icon')}
      </button>
      ${lightupEntryButton()}
    </header>
    <section class="seed-insight-strip" aria-label="种草概览">
      ...
      <article><span>推荐景点</span><b>${hero.spot}</b><em>最热</em></article>
    </section>
    <section class="home-block recommend-block">
      ...
      <article class="recommend-card" data-open="${hero.contentId}">
        <img class="hero-base-img" src="${hero.image}" alt="" />
        <img class="hero-layer-img" src="${hero.image}" alt="" />
        <div class="hero-scrim"></div>
        <span class="duration-chip">${hero.duration}</span>
        ${collectButton(hero.contentId)}
        <div class="recommend-info">
          <p>${hero.kicker}</p>
          <div class="hero-tags">${hero.tags.map(tag => `<span>${tag}</span>`).join('')}</div>
          <h3>${hero.title}</h3>
...
function contentCard(item, index = 0) {
  ...
  const image = item.image || `./assets/figma-seed-home/${images[index % images.length]}.png`;
  ...
  return `
    <article class="seed-card ${item.tone}" data-open="${contentId}">
      <img src="${image}" alt="" />
      <div class="card-gradient"></div>
      ${collectButton(contentId, 'mini')}
      <div class="card-copy">
        <h3>${item.title}</h3>
...
function collectButton(id, extraClass = '') {
  ...
  return `
    <button class="collect-btn ${extraClass} ${isSaved ? 'saved' : ''}" data-collect-content="${id}" aria-label="${isSaved ? '取消种草' : '加入种草'}">
      ${iconImg('collect.svg', 'collect-icon')}
    </button>
  `;
}
...
function lightupEntryButton() {
  return `
    <button class="lightup-entry-btn ${state.lightupPanelOpen ? 'active' : ''}" data-open-lightups aria-label="打开收藏点亮">
      ${iconImg('amenties_special.svg', 'header-icon')}
    </button>
  `;
}
  ...
  return `
    ...
    <section class="search-sheet" role="dialog" aria-label="搜索内容">
      <div class="search-top">
        <h2>搜索文化种草</h2>
        <button data-close-search aria-label="关闭搜索">${iconImg('close.svg', 'close-icon')}</button>
      </div>
...
function renderDetail() {
  ...
  screen.innerHTML = `
    ...
    ${systemBar('detail-v3-system')}
    <header class="detail-appbar detail-v3-appbar">
      <button data-back aria-label="返回">${iconImg('返回.svg', 'back-icon invert')}</button>
      <div></div>
      <div class="detail-appbar-actions">
        ...
        <button data-open-sheet="share-detail" aria-label="分享">${iconImg('share.svg', 'share-icon')}</button>
      </div>
    </header>
...
function detailPaintingBreakdown() {
  const activeHotspot = hotspots.find(hotspot => hotspot.id === state.activeHotspotId) || hotspots[0];
  const activeIndex = Math.max(0, hotspots.findIndex(hotspot => hotspot.id === activeHotspot.id));
  const activeImage = activeIndex === 0
    ? './assets/figma-painting-tab/hotspot-aoshan.png'
    : `./assets/figma-painting-tab/spot-${activeIndex + 1}.png`;
  return `
    <section class="detail-v3-block detail-painting-breakdown">
      ...
      <div class="detail-painting-card detail-v3-painting-card">
        <div class="detail-painting-art">
          ...
          ${hotspots.map(hotspot => `
            <button class="${hotspot.id === activeHotspot.id ? 'focused' : ''}" style="left:${hotspot.x}%;top:${hotspot.y}%;" data-detail-hotspot="${hotspot.id}" aria-label="${hotspot.name}" aria-pressed="${hotspot.id === activeHotspot.id}">
      ...
      <article class="detail-v3-hotspot-card">
        ...
        <div>
          <b>${activeHotspot.dim} · ${activeHotspot.name}</b>
...
function renderPreference() {
  ...
  const hasCurrentSelection = Boolean(state.preference[activeGroup.key]);
  const stepLabels = ['身份视角', '预留时间', '预期氛围'];
  screen.innerHTML = `
    <section class="preference-flow" data-pref-step="${step}">
      ${systemBar('preference-system')}
      <header class="preference-nav">
        <button class="preference-round-btn" data-back aria-label="返回">${iconImg('返回.svg', 'back-icon')}</button>
        <h1>偏好设置</h1>
      ...
      <div class="preference-actions ${step === 1 ? 'single' : ''}">
        ...
        <button class="preference-primary-action" ${step === groups.length ? 'data-confirm-pref' : 'data-pref-next'} ${hasCurrentSelection ? '' : 'disabled aria-disabled="true"'}>
...
function renderGenerationLoading() {
  ...
  const selectedPreference = plan.selectedPreference;
  const loadingSteps = [
    { label: '解析偏好', desc: `${selectedPreference.role} · ${selectedPreference.days} · ${selectedPreference.vibe}` },
    { label: '匹配文化线索', desc: plan.includedHotspots.length ? `${plan.includedHotspots.length} 个画中线索已接入` : '从灯会、技艺与路线中提取灵感' },
    { label: '生成体验行程', desc: '正在整理时间线、互动和点亮内容' },
  ];
  screen.innerHTML = `
    <div class="generation-loading-page" aria-live="polite">
      ${systemBar('result-system-bar')}
      <header class="loading-appbar">
        <button data-back aria-label="返回">${iconImg('返回.svg', 'back-icon invert')}</button>
      </header>
      <section class="loading-orbit" aria-hidden="true">
        <span></span>
        <i></i>
        <b></b>
      </section>
      <section class="loading-copy">
        <h1>正在生成你的<br />专属体验行程</h1>
      </section>
      <section class="loading-progress-card">
        <div>
          <b>预计 3 秒</b>
          <span>即将进入体验结果页</span>
        </div>
      ...
      <section class="loading-step-list" aria-label="生成进度">
        ${loadingSteps.map((step, index) => `
          <article style="--delay:${index};">
            <em>${index + 1}</em>
            <div>
              <b>${step.label}</b>
              <p>${step.desc}</p>
  ...
  screen.innerHTML = `
    <div class="agent-result-page">
      ...
      <header class="agent-appbar result">
        ...
        <span>定制体验</span>
        <div>
          <button data-open-sheet="share-result" aria-label="分享">${iconImg('share.svg', 'share-icon')}</button>
        </div>
      </header>
...
function globalLightupPanel() {
  ...
  return `
    ...
    <aside class="global-lightup-panel" role="dialog" aria-label="收藏点亮清单">
      <header>
        <h2>我的文化沉淀</h2>
        <button data-close-lightups aria-label="关闭收藏点亮清单">${iconImg('close.svg', 'close-icon')}</button>
      </header>
...
function globalLightupGroup(group) {
  const expandedGroups = state.expandedLightupGroups || [];
  const isExpanded = expandedGroups.includes(group.kind);
  const primaryItems = group.items.slice(0, 2);
  const extraItems = group.items.slice(2);
  const canToggle = group.items.length > 2;
  return `
    <section class="global-lightup-group ${isExpanded ? 'is-expanded' : ''}" data-lightup-group="${group.kind}">
      <div class="global-lightup-group-head">
        <h3>${group.title}</h3>
      </div>
      <div class="global-lightup-items">
        ${primaryItems.map(item => globalLightupItem(item)).join('')}
        ${canToggle ? `
          <div class="global-lightup-extra ${isExpanded ? 'is-expanded' : ''}" data-lightup-extra>
            ${extraItems.map(item => globalLightupItem(item)).join('')}
          </div>
        ` : ''}
        ${canToggle ? `
          <button class="global-lightup-more ${isExpanded ? 'is-expanded' : ''}" type="button" data-toggle-lightup-group="${group.kind}" aria-expanded="${isExpanded}">
            ${isExpanded ? '收起' : '查看更多'} <span>${isExpanded ? '⌃' : '⌄'}</span>
          </button>
        ` : ''}
      </div>
    </section>
  ...
  return `
    ...
    <aside class="secondary-sheet ${content.size || ''}" role="dialog" aria-label="${content.title}">
      ...
      <header class="secondary-head">
        ...
        <button data-close-sheet aria-label="关闭">${iconImg('close.svg', 'close-icon')}</button>
      </header>
...
function getSecondarySheetContent(type = '') {
      ...
      body: `
        <div class="secondary-location-list">
          ${destinationOptions.map(item => `
            <button data-select-destination="${item.contentId}">
              <b>${item.location}</b>
  ...
  if (type === 'seed-recommend') {
    return {
      ...
      body: `<div class="secondary-card-list">${getFilteredContents({ ...state, seedFilter: '全部' }).slice(0, 4).map(secondaryContentCard).join('')}</div>`,
    };
  }
...
function secondaryContentCard(item, index = 0) {
  ...
  const contentId = item.contentId || item.id;
  return `
    <article class="secondary-content-card" data-open="${contentId}">
      <img src="${image}" alt="" />
      <div>
...
function getGlobalLightupGroups() {
  ...
  const scenicItems = contents.map(item => ({
    title: `${item.location.replace('江西 · ', '')} · ${item.title}`,
    desc: item.subtitle,
    image: `./assets/figma-seed-home/${contentImages[item.id] || 'article-1'}.png`,
    action: `data-open="${item.id}"`,
  }));
  const paintingItems = [
    {
      title: paintingData.title,
    ...
    ...hotspots.slice(0, 3).map((hotspot, index) => ({
      title: `${hotspot.name} · ${hotspot.dimLabel.split('·')[0].trim()}`,
      desc: hotspot.desc,
      image: index === 0
        ? './assets/figma-painting-tab/hotspot-aoshan.png'
        : `./assets/figma-painting-tab/spot-${index + 1}.png`,
      action: 'data-jump-tab="painting"',
    })),
  ];
  const extraSkillItems = [
    { title: '鱼灯扎制', desc: '把灯街鱼灯转成可带走的夜游纪念。', image: './assets/figma-seed-home/detail-trip-1.png' },
    { title: '祈愿木牌', desc: '用手写愿望完成行程最后的情绪收束。', image: './assets/figma-seed-home/img-05.png' },
    { title: '灯影拓印', desc: '将灯彩图样拓成一张可收藏的纹样纸。', image: './assets/figma-seed-home/detail-hotspot.png' },
    { title: '市集糖塑', desc: '用糖线勾勒节庆图案，适合短暂停留体验。', image: './assets/figma-seed-home/article-5.png' },
    { title: '香囊缝制', desc: '把草木气味和旅行记忆缝进随身小物。', image: './assets/figma-seed-home/img-03.jpg' },
    { title: '花灯绘色', desc: '在基础灯架上完成配色、描线和点灯动作。', image: './assets/figma-seed-home/img-04.jpg' },
    { title: '纹样印章', desc: '把古画纹样拆成印章，形成轻量打卡物。', image: './assets/figma-seed-home/img-06.png' },
  ];
  const skillItems = [
    ...skills.map((skill, index) => ({
      title: skill.name,
      desc: skill.desc,
      image: `./assets/figma-seed-home/${index === 0 ? 'article-2' : index === 1 ? 'article-6' : 'img-02'}.png`,
      action: 'data-jump-tab="experience"',
    })),
    ...extraSkillItems.map(item => ({ ...item, action: 'data-jump-tab="experience"' })),
  ];
  return [
    {
      kind: 'scenic',
      title: '收藏景点',
      total: scenicItems.length,
      items: scenicItems,
    },
    {
      kind: 'painting',
      title: '点亮古画',
      total: paintingItems.length,
      items: paintingItems,
    },
    {
      kind: 'skill',
...
function renderPainting() {
  ...
  const activePaintingIndex = ((state.paintingIndex || 0) % paintingViews.length + paintingViews.length) % paintingViews.length;
  ...
  screen.innerHTML = `
    ...
    <section class="home-block related-block">
      ...
      <div class="related-grid">
        ${relatedSpots.map((item, index) => relatedCard(item, index, activePaintingIndex)).join('')}
      </div>
    </section>
  `;
}
      ...
      <div class="painting-knowledge-main">
        ...
        <div class="painting-knowledge-copy">
          <div class="painting-knowledge-title">
            ...
            <div>
              <button data-add-hotspot="${hotspot.id}" aria-label="添加文化线索">${iconImg('add_f.svg', 'add-icon')}</button>
            </div>
          </div>
          <p>${hotspot.desc}</p>
        </div>
...
function relatedCard(item, index = 0, paintingIndex = 0) {
  const image = item.image || getPaintingRelatedImage(paintingIndex, index);
  return `
...
function renderExperience() {
  const [province, city] = getDestinationDisplay(state.selectedContentId);
  const destination = `${province}·${city}`;
  const lightedCount = state.lightups.skills.length;
  const mode = getExperienceMode(state);
  const experienceSummary = getExperienceDestinationSummary(state);
  ...
  screen.innerHTML = `
    ...
    <section class="experience-hero-card">
      <img src="./assets/experience-new/experience-hero.png" alt="" />
      <div class="experience-hero-shade"></div>
      <div class="experience-hero-copy">
        <span>${experienceSummary.tag}</span>
        <h2>${experienceSummary.title}</h2>
        <p>${experienceSummary.desc}</p>
      </div>
      <div class="experience-status-row">
        <span><b>${route.length + 1}</b><em>个节点</em></span>
        <span><b>${lightedCount}</b><em>已点亮</em></span>
        <span><b>${experienceSummary.bestTime}</b><em>最佳入场</em></span>
      </div>
    </section>
  ...
  return `
    <article class="experience-product-card ${product.tone}">
      <div class="experience-product-visual">
        <img src="./assets/experience-new/${images[index % images.length]}.png" alt="" />
      </div>
  ...
  return `
    <article class="experience-skill-card ${isLighted ? 'is-lighted' : ''}">
      <div class="experience-skill-main">
        <img class="skill-thumb" src="./assets/experience-new/${images[skill.id] || 'skill-1'}.png" alt="" />
        <div>
          <span>${skill.meta} · ${skill.duration}</span>
...
function profileDemoTrips() {
  return [
    { title: '千灯夜游', location: '葛仙村', role: '游客', days: '一天', vibe: '出片打卡', image: 'hero-1.png' },
    { title: '画中游览', location: '葛仙村', role: '游客', days: '两天', vibe: '安静文艺', image: 'hero-6.jpg', fromPainting: true },
    { title: '非遗体验', location: '葛仙村', role: '匠人', days: '一天', vibe: '热闹节庆', image: 'hero-4.jpg' },
    { title: '夜市烟火', location: '葛仙村', role: '商贩', days: '半天', vibe: '热闹节庆', image: 'img-07.jpg' },
    { title: '水边慢游', location: '葛仙村', role: '游客', days: '一天', vibe: '安静文艺', image: 'hero-3.jpg' },
    { title: '剪纸轻旅', location: '葛仙村', role: '匠人', days: '半天', vibe: '安静文艺', image: 'img-05.png' },
    { title: '亲子找灯', location: '葛仙村', role: '游客', days: '一天', vibe: '热闹节庆', image: 'img-04.jpg' },
    { title: '祈愿收束', location: '葛仙村', role: '游客', days: '半天', vibe: '出片打卡', image: 'img-10.jpg' },
  ].map((item, demoIndex) => ({ ...item, demoIndex }));
}

function profileMyTripCard(item, index = 0) {
  const image = item.image || ['hero-1.png', 'hero-6.jpg', 'hero-4.jpg'][index % 3];
  const imageUrl = seedHomeImg(image);
  ...
  return `
    <article class="profile-my-trip-card" ${actionAttr} style="--trip-image:url('${imageUrl}');">
      <span>${location}</span>
      <h3>${item.title}</h3>
...
function profileRecommendCard(item, index) {
  const images = index === 0 ? ['hero-6.jpg', 'img-04.jpg'] : ['hero-3.jpg', 'img-11.jpg'];
  return `
    <article class="profile-recommend-card" data-preview-recommend="${index}">
      <div class="profile-recommend-visual" aria-hidden="true">
        <img src="${seedHomeImg(images[0])}" alt="" />
        <img src="${seedHomeImg(images[1])}" alt="" />
      </div>
...
function profileSavedItem(item) {
  return `
    <article data-open="${item.id}" data-close-profile>
      <img src="./assets/figma-seed-home/${item.id === 'festival' ? 'hero-1' : item.id === 'craft' ? 'article-2' : item.id === 'village' ? 'article-3' : 'article-1'}.png" alt="" />
      <div>
...
function routeGenerate(source) {
  ...
  if (getGenerationRoute(state) === 'preference') {
    ...
    beginGenerationLoading(state, source);
  }
  render();
}

function startGenerationLoading(source) {
  beginGenerationLoading(state, source || state.pendingSource || 'detail');
  state.preferenceStep = 1;
  render();
}

function syncGenerationLoadingTimer() {
  if (state.page !== 'loading') {
    clearTimeout(generationLoadingTimer);
    generationLoadingTimer = null;
    return;
  }

  if (generationLoadingTimer) return;
  generationLoadingTimer = setTimeout(() => {
    generationLoadingTimer = null;
    if (state.page !== 'loading') return;
    state.page = state.loadingTargetPage || 'result';
    state.loadingTargetPage = null;
    render();
  }, GENERATION_LOADING_DURATION_MS);
}
...
document.addEventListener('click', event => {
  ...
  if (addSelectedHotspot) {
    ...
    const alreadyAdded = state.sessionHotspots.includes(id);
    if (!alreadyAdded) toggleHotspot(state, id);
    state.activeHotspotId = id;
    showToast(alreadyAdded ? '已在本次探画线索中' : '已加入本次探画线索');
    render();
    return;
  }
  ...
  if (event.target.closest('[data-pref-next]')) {
    ...
    const step = Math.min(Math.max(state.preferenceStep || 1, 1), groups.length);
    const activeGroup = groups[step - 1];
    if (!state.preference[activeGroup.key]) return;
  ...
  if (confirmPref) {
    if (!confirmPref.classList.contains('preference-skip')) {
      const groups = getPreferenceGroups();
      const step = Math.min(Math.max(state.preferenceStep || 1, 1), groups.length);
      const activeGroup = groups[step - 1];
      if (!state.preference[activeGroup.key]) return;
    }
    startGenerationLoading(state.pendingSource || 'detail');
    return;
  }
  ...
  const toggleLightupGroup = event.target.closest('[data-toggle-lightup-group]');
  if (toggleLightupGroup) {
    const kind = toggleLightupGroup.dataset.toggleLightupGroup;
    const expandedGroups = state.expandedLightupGroups || [];
    const willExpand = !expandedGroups.includes(kind);
    state.expandedLightupGroups = willExpand
      ? [...expandedGroups, kind]
      : expandedGroups.filter(item => item !== kind);
    const group = toggleLightupGroup.closest('[data-lightup-group]');
    group?.classList.toggle('is-expanded', willExpand);
    group?.querySelector('[data-lightup-extra]')?.classList.toggle('is-expanded', willExpand);
    toggleLightupGroup.classList.toggle('is-expanded', willExpand);
    toggleLightupGroup.setAttribute('aria-expanded', String(willExpand));
    toggleLightupGroup.innerHTML = `${willExpand ? '收起' : '查看更多'} <span>${willExpand ? '⌃' : '⌄'}</span>`;
    return;
  }
