...
test('generation loading state prepares automatic result transition', () => {
  const state = createState();
  assert.equal(typeof modelApi.beginGenerationLoading, 'function');
  const nextPage = modelApi.beginGenerationLoading(state, 'explore');

  assert.equal(modelApi.GENERATION_LOADING_DURATION_MS, 3000);
  assert.equal(nextPage, 'result');
  assert.equal(state.page, 'loading');
  assert.equal(state.loadingTargetPage, 'result');
...
test('generated plan shows all five culture dimensions without painting hotspot input', () => {
  ...
  assert.deepEqual(plan.focusDimensions.map(item => item.dim), ['物', '艺', '人', '景', '情']);
  assert.deepEqual(plan.selectedPreference, { role: '游客', days: '两天一夜', vibe: '安静文艺' });
});
...
test('painting related cards use the uploaded 1-12 image pool by painting', () => {
  const firstPaintingImages = [0, 1, 2, 3].map(index => getPaintingRelatedImage(0, index));
  const secondPaintingImages = [0, 1, 2, 3].map(index => getPaintingRelatedImage(1, index));

  assert.deepEqual(firstPaintingImages, [
    './assets/figma-painting-tab/1.jpg',
    './assets/figma-painting-tab/5.jpg',
    './assets/figma-painting-tab/9.jpg',
    './assets/figma-painting-tab/3.jpg',
  ]);
  assert.equal(secondPaintingImages[0], './assets/figma-painting-tab/7.jpg');
  assert.notDeepEqual(firstPaintingImages, secondPaintingImages);
  assert.ok([...firstPaintingImages, ...secondPaintingImages].every(image => /figma-painting-tab\/(?:[1-9]|1[0-2])\.jpg$/.test(image)));
});
...
test('destination switcher offers diverse exploration locations', () => {
  const provinces = new Set(destinationOptions.map(item => item.location.split(' · ')[0]));

  assert.equal(destinationOptions.length >= 6, true);
  assert.equal(provinces.size >= 5, true);
  assert.equal(destinationOptions.every(item => item.contentId && item.title && item.meta), true);
});

test('destination display follows selected exploration destination', () => {
  assert.deepEqual(getDestinationDisplay('market'), ['河南', '开封']);
  assert.deepEqual(getDestinationDisplay('village'), ['江苏', '苏州']);
  assert.deepEqual(getDestinationDisplay('unknown'), ['江西', '葛仙村']);
});

test('seed home recommendation and discover cards follow selected destination', () => {
  const state = createState();
  state.selectedContentId = 'market';

  const hero = getSeedHeroRecommendation(state.selectedContentId);
  const discover = getSeedDiscoverContents(state);

  assert.equal(hero.title, '开封 · 汴梁夜市');
  assert.equal(hero.image, './assets/figma-seed-home/hero-2.jpg');
  assert.equal(discover[0].title, '汴梁夜市');
  assert.equal(discover[0].image, './assets/figma-seed-home/img-07.jpg');
  assert.notEqual(discover[0].title, '千灯入夜');

  state.selectedContentId = 'village';
  const villageDiscover = getSeedDiscoverContents(state);
  assert.equal(getSeedHeroRecommendation(state.selectedContentId).image, './assets/figma-seed-home/hero-3.jpg');
  assert.equal(villageDiscover[0].title, '平江水巷');
  assert.equal(villageDiscover[0].image, './assets/figma-seed-home/img-11.jpg');
});
...
test('experience tab content follows selected destination', () => {
  const state = createState();
  state.selectedContentId = 'market';

  assert.equal(getExperienceDestinationSummary(state).title, '汴梁夜市后的宋风文化体验单');
  assert.equal(getExperienceDestinationSummary(state).bestTime, '18:40');
  assert.equal(getExperienceRoute(state)[0].title, '入市取景');

  state.selectedContentId = 'village';
  assert.equal(getExperienceDestinationSummary(state).title, '平江水巷里的江南慢游体验单');
  assert.equal(getExperienceRoute(state)[1].title, '桥巷慢游');
});
